    ...
    <div v-else class="add-destination-form">
        ...
        <form>
            ...
            <div class="grid-group">
                ...
                <span v-if="guides.length > 0">
                    ...
                    <select id="destinationGuide" name="destinationGuide" v-model="destination.guideId">
                        <option v-for="guide in guides" :key="guide.guideId" :value="guide.guideId">
                            {{ guide.name }} {{guide.lastName }}</option>
                    </select>
                </span>
            </div>
            ...
            <div class="button-center">
                <button @click="updateDestination" type="button" class="btn secondary">Update Destination</button>
            </div>
        </form>
    </div>
</template>
...
export default {
    components: { Loading },
    props: ['oldDestination', 'getUpdatedDestinations'],
    data() {
        return {
            loading: true,
            guides: [],
    ...
    mounted() {
        ...
        this.destination.name = this.oldDestination.name;
        this.destination.description = this.oldDestination.description;
        this.destination.price = this.oldDestination.price;
        this.destination.groupSize = this.oldDestination.groupSize;
        this.destination.startDate = this.oldDestination.startDate;
        this.destination.endDate = this.oldDestination.endDate;
        this.destination.guideId = this.oldDestination.guideId;
        this.destination.cityName = this.oldDestination.cityName;
        this.destination.countryName = this.oldDestination.countryName;
        this.destination.difficultyLevelId = this.oldDestination.difficultyLevelId;
        this.destination.continentId = this.oldDestination.continentId;

        console.log(this.oldDestination);
    },
    methods: {
        updateDestination() {
            ...
            this.loading = true;
            axios
                .put(`http://localhost:8080/api/v1/destinations/update/${this.oldDestination.destinationId}`, this.destination)
                .then(response => {
                    this.loading = false;
                    this.getUpdatedDestinations();
                    this.$emit('close');
                })
